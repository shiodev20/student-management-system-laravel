...
<div class="row">
  ...
  <div class="col">
    <div class="card">
      <div class="card-body">
        <div class="card-title">Tổng kết môn học</div>

        <form action="{{ route('reports.processSubject') }}" method="POST" id="reportSubjectForm">
          @csrf
...
@push('js')
  <script>
    const reportForm = document.querySelector('#reportSubjectForm')
    ...
    .then(data => {
      if(!data.status) {
        for (const message in data.messages) {
          document.querySelector(`#reportSubjectForm select[name=${message}]`).style.border =  '1px solid #dc3545';
          document.querySelector(`#reportSubjectForm .${message}-error`).style.display = 'block';
          document.querySelector(`#reportSubjectForm .${message}-error`).innerHTML = data.messages[message];
        }

        document.querySelector('#reportSubjectForm #submit-button').classList.add('align-self-center', 'mt-1');
      }
      else {
        ...
        data.result.grades.forEach((grade, idx) => {
          ...
          data.result.subjectReports[grade.id].forEach(classroom => {
            report += `
              <tr>
...
@push('js')
<script>
  const selects = document.querySelectorAll('#reportSubjectForm select')
